...
export default {
  ...
  data() {
    return {
      index: 1,
      routeHint: this.$route.params.tag,
    };
  },
  methods: {
    ...mapActions(["set"]),
  ...
  computed: {
    ...
    hintsArray() {
      return this.hints.map((cat) => cat.hint);
    },
    hint() {
      let res = "ERROR";
      this.hintsArray.forEach((cat) => {
        if (cat.toLowerCase() == this.routeHint) {
          res = cat;
        }
      });
      return res;
    ...
    activeArticles() {
      return this.articles.filter((article) => {
        if (!article.hints) {
          ...
          return article.hints.includes(this.hint);
        }
      });
    },
    pages() {
